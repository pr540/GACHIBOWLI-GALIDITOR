    ...
    <main className="relative min-h-screen flex items-center justify-center p-4 sm:p-6 bg-[--color-canvas] text-[--color-text] overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-lg rounded-2xl border border-[--color-line-bright] bg-[--color-surface]/95 backdrop-blur-md p-6 sm:p-8 shadow-2xl">
        ...
        <div className="mt-5 text-center">
          <span className="text-[10px] font-mono uppercase tracking-wider px-2 py-0.5 rounded bg-[--color-brass-dim] text-[--color-brass] font-semibold border border-[--color-brass]/30">
            SplitOps Authentication
          </span>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-[--color-text] mt-2">
            Sign In to SplitOps
          </h1>
        ...
        <div className="mt-6">
          <button
            ...
            className="w-full py-3 px-4 rounded-[--radius] border border-[--color-line-bright] bg-[--color-surface-raised] hover:border-[--color-brass] text-sm font-semibold text-[--color-text] flex items-center justify-center gap-3 transition-all shadow-sm active:scale-[0.99] cursor-pointer"
          >
            {isGoogleSigningIn ? (
              <span className="flex items-center gap-2 text-xs font-medium text-[--color-text]">
        ...
        <div className="relative my-6 text-center">
          ...
          <span className="relative px-3 text-[11px] uppercase tracking-wider bg-[--color-surface] text-[--color-muted] font-medium">
        ...
        <div className="relative mb-3">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-2 rounded-[--radius] border border-[--color-line-bright] bg-[--color-canvas] text-xs text-[--color-text] outline-none transition-colors focus:border-[--color-brass]"
          />
        </div>
          ...
          {filteredMembers.map((member) => {
            ...
            return (
              <button
                ...
                className={`p-2.5 rounded-[--radius] border text-left transition-all text-xs flex items-center justify-between cursor-pointer ${
                  isSelected
                    ? "border-[--color-brass] bg-[--color-surface-raised] font-semibold text-[--color-brass] ring-1 ring-[--color-brass]"
                    : "border-[--color-line] bg-[--color-surface-raised] text-[--color-text] hover:border-[--color-brass]/70 hover:bg-[--color-surface]"
                }`}
              >
                <span className="truncate font-medium">{member.name}</span>
                {isOwner ? (
                  <span className="text-[9px] uppercase px-1.5 py-0.2 rounded bg-[--color-brass] text-[#0b0e0d] font-bold shrink-0">
                    Owner
                  </span>
                ) : isSelected ? (
                  <span className="text-[11px] text-[--color-credit] font-bold">✓</span>
                ) : null}
              </button>
            );
          })}
        ...
        <form onSubmit={handleCustomLogin} className="mt-5 pt-4 border-t border-[--color-line] flex gap-2">
          <input
            ...
            className="flex-1 rounded-[--radius] border border-[--color-line-bright] bg-[--color-canvas] px-3 py-2 text-xs text-[--color-text] outline-none transition-colors focus:border-[--color-brass]"
          />
          <button
            type="submit"
            disabled={!customName.trim()}
            className="px-4 py-2 rounded-[--radius] bg-[--color-brass] text-xs font-semibold text-[#0b0e0d] transition-opacity hover:opacity-90 disabled:opacity-40 shrink-0 cursor-pointer"
          >
            Enter →
          </button>
        </form>
          ...
          <div className="w-full max-w-sm rounded-2xl border border-[--color-line-bright] bg-[--color-surface] p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between pb-2 border-b border-[--color-line]">
              <div className="flex items-center gap-2">
                ...
                <span className="font-semibold text-sm text-[--color-text]">Sign in with Google</span>
              </div>
              <button
                onClick={() => setShowGoogleModal(false)}
                className="text-xs text-[--color-muted] hover:text-[--color-text] p-1"
              >
                ✕
              </button>
            </div>
            ...
            <div className="space-y-2">
              <button
                ...
                className="w-full p-3 rounded-[--radius] border border-[--color-line] hover:border-[--color-brass] bg-[--color-surface-raised] text-left text-xs flex items-center justify-between transition-colors cursor-pointer"
              >
                <div className="flex items-center gap-2.5">
                  <div className="w-7 h-7 rounded-full bg-[--color-brass] text-[#0b0e0d] flex items-center justify-center font-bold text-xs">
                    P
                  </div>
                  <div>
                ...
                <span className="text-[11px] text-[--color-brass] font-medium">Select →</span>
              </button>
              ...
              <div className="pt-2">
                <input
                  ...
                  className="w-full rounded-[--radius] border border-[--color-line-bright] bg-[--color-canvas] px-3 py-2 text-xs text-[--color-text] outline-none transition-colors focus:border-[--color-brass]"
                />
                <button
                  onClick={() => handleGoogleSignIn(googleEmail)}
                  disabled={!googleEmail.trim()}
                  className="w-full mt-2 py-2 rounded-[--radius] bg-[--color-brass] text-xs font-semibold text-[#0b0e0d] transition-opacity hover:opacity-90 disabled:opacity-40 cursor-pointer"
                >
